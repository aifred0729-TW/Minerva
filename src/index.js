...
// This MUST run before any React/CRA code to intercept the error before
// react-error-overlay registers its own handler.
// "ResizeObserver loop completed with undelivered notifications" is a benign
// browser warning (not a real error) that occurs frequently with ReactFlow.
// See: https://github.com/WICG/resize-observer/issues/38
if (typeof window !== 'undefined') {
    // Capture phase listener — fires BEFORE react-error-overlay's bubble listener
    ...
    // Also patch window.onerror as a fallback
...
// Helper: detect session/JWT expiration from error message or code
const isSessionExpiredError = (code, message) => {
...
// Debounce: only trigger session-expired redirect once
let _sessionExpiredTriggered = false;
const handleSessionExpired = (source) => {
    if (_sessionExpiredTriggered) return;
    _sessionExpiredTriggered = true;
    console.log(`[Session Expired] detected via ${source}, logging out`);
    snackActions.warning('Session expired — logging out');
    FailedRefresh(true);
    // Small delay so the snack is visible before redirect
    setTimeout(() => {
  ...
  try{
    if (graphQLErrors) {
      ...
      for (let err of graphQLErrors) {
        ...
        // Check for session/JWT expiration first
        if (isSessionExpiredError(code, err.message)) {
            handleSessionExpired('graphQLError: ' + (code || err.message));
            return;
        }
        // Also treat access-denied that mentions JWT as session expired
        if (code === 'access-denied' && err.message && err.message.toLowerCase().includes('jwt')) {
    ...
    if (networkError) {
      ...
      // Check for session expiration in network errors
      const netCode = networkError.extensions?.code;
      const netMsg = networkError.message || '';
      if (isSessionExpiredError(netCode, netMsg)) {
          handleSessionExpired('networkError: ' + (netCode || netMsg));
          return;
      }
      // HTTP 401/403 without extensions usually means expired session
      if (networkError.statusCode === 401 || networkError.statusCode === 403) {
          handleSessionExpired('networkError: HTTP ' + networkError.statusCode);
          return;
      }
