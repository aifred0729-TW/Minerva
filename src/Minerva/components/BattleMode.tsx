...
function getTargetColors(mode: OperationMode, themeMode: 'dark' | 'light'): { signal: number[], accent: number[] } {
  switch (mode) {
    case 'combat':
    ...
    default:
      return themeMode === 'light'
        ? { signal: LIGHT_SIGNAL, accent: LIGHT_ACCENT }
        : { signal: DARK_SIGNAL, accent: DARK_ACCENT };
  }
}
...
export const BattleMode: React.FC = () => {
  ...
  const normalSignal = theme === 'light' ? LIGHT_SIGNAL : DARK_SIGNAL;
  const normalAccent = theme === 'light' ? LIGHT_ACCENT : DARK_ACCENT;
  const currentSignalRef = useRef([...normalSignal]);
  const currentAccentRef = useRef([...normalAccent]);

  // Initialize colors on mount and theme change
  useEffect(() => {
    if (mode === 'normal') {
  ...
  useEffect(() => {
    const { signal: targetSignal, accent: targetAccent } = getTargetColors(mode, theme);
