...
import { absoluteDownloadUrl, directDownloadUrl } from '../../lib/urls';
import {
    Download,
    PlusCircle,
    Box,
...
export function PayloadsList() {
  ...
  const [renameTarget, setRenameTarget] = useState<unknown>(null);
  const [descriptionTarget, setDescriptionTarget] = useState<unknown>(null);
  ...
  // Queries & Mutations
  const { data, loading, subscribeToMore } = useQuery<any>(GET_PAYLOADS_LIST, {
  ...
  const [triggerRebuild] = useMutation<any>(REBUILD_PAYLOAD_MUTATION, {
  ...
  const [toggleDelete] = useMutation<any>(TOGGLE_PAYLOAD_DELETE_MUTATION, {
      onCompleted: () => { /* Subscription handles update */ }
  });

  const [updateFilename] = useMutation<any>(UPDATE_FILENAME_MUTATION, {
      onCompleted: () => { /* Subscription handles update */ }
  });
  
  const [updateDescription] = useMutation<any>(UPDATE_PAYLOAD_DESCRIPTION, {
      onCompleted: () => { /* Subscription handles update */ }
  });

  const [updateAlerts] = useMutation<any>(UPDATE_PAYLOAD_ALERTS); 
  const [updateAllowed] = useMutation<any>(UPDATE_PAYLOAD_ALLOWED);

  // Lazy Queries for "Generate" actions
  const [exportConfig] = useLazyQuery<any>(EXPORT_PAYLOAD_CONFIG, {
      onCompleted: (data: any) => {
  ...
  const [generateRedirectRules] = useLazyQuery<any>(GENERATE_REDIRECT_RULES, {
      fetchPolicy: "network-only",
      onCompleted: (data: any) => {
  ...
  const [checkConfig] = useLazyQuery<any>(CHECK_PAYLOAD_CONFIGURATION, {
      fetchPolicy: "network-only",
      onCompleted: (data: any) => {
          setInfoModalLoading(false);
  ...
  const [generateIOC] = useLazyQuery<any>(GENERATE_IOC, {
      fetchPolicy: "network-only",
      onCompleted: (data: any) => {
          setInfoModalLoading(false);
  ...
  const [generateSample] = useLazyQuery<any>(GENERATE_SAMPLE_MESSAGE, {
      fetchPolicy: "network-only",
      onCompleted: (data: any) => {
          setInfoModalLoading(false);
  ...
  const activePayload = useMemo(() => {
      ...
      return payloads.find((p: Record<string, unknown>) => p.id === (selectedPayload as Record<string, unknown>).id) || selectedPayload;
              ...
              if (data?.updatePayload?.status === 'success') {
                  ...
                  try {
                      ...
                      if (existingData && existingData.payload) {
                          const newPayloads = existingData.payload.filter((p: Record<string, unknown>) => p.uuid !== uuid);
                          cache.writeQuery({
                              query: GET_PAYLOADS_LIST,
                              variables: queryVars,
                              data: {
                    ...
                    <AnimatePresence>
                          ...
                          >
                              <div className="py-1">
                                  ...
                                  <button onClick={() => { setIsOpen(false); navigate('/payloads/new'); /* Enhancement: pre-fill config from this payload */ }} className="action-item">
        ...
        <div className="flex items-center gap-2 justify-end">
            <button
              onClick={(e) => { 
                  ...
                  if (row.filemetum?.agent_file_id) {
                      const link = absoluteDownloadUrl(row.filemetum.agent_file_id);
            ...
            <button
              onClick={(e) => { 
                  e.stopPropagation(); 
                  if (row.filemetum?.agent_file_id) {
                      window.location.href = directDownloadUrl(row.filemetum.agent_file_id);
                  }
              }}
              className="p-2 text-gray-400 hover:text-signal hover:bg-white/10 rounded transition-all disabled:opacity-30 disabled:cursor-not-allowed"
              disabled={row.build_phase !== 'success' || !row.filemetum?.agent_file_id}
    ...
    <div className="min-h-screen bg-void text-signal font-sans p-6 lg:p-12">
      ...
      <header className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-widest text-white uppercase">PAYLOADS OVERVIEW</h1>
