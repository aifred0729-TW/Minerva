...
export function PayloadsList() {
  ...
  const [pageData, __setPageData] = React.useState({
    totalCount: 0,
    fetchLimit: 20,
    showDeleted: false,
  ...
  // Queries & Mutations
  const { data, loading, subscribeToMore } = useQuery(GET_PAYLOADS_LIST, {
  ...
  const activePayload = useMemo(() => {
      ...
      return payloads.find((p: Record<string, unknown>) => p.id === selectedPayload.id) || selectedPayload;
