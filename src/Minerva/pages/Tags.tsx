...
const Tags = () => {
    ...
    const [importTagtypes] = useMutation(IMPORT_TAGTYPES, {
        ...
        onError: () => {
            snackActions.error('Failed to import tag types');
        }
    });
    ...
    const [exportTagtypes] = useLazyQuery(EXPORT_TAGTYPES, {
        ...
        onError: () => {
            snackActions.error('Failed to export tag types');
        }
    });
    ...
    const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (ev) => {
            try {
                importTagtypes({ variables: { tagtypes: String(ev.target?.result) } });
            } catch {
    ...
    const filteredTagtypes = useMemo(() => {
        if (!searchFilter) return tagtypes;
        const lower = searchFilter.toLowerCase();
        return tagtypes.filter(t => t.name.toLowerCase().includes(lower) || t.description.toLowerCase().includes(lower));
    }, [tagtypes, searchFilter]);
                ...
                <div className="h-16 border-b border-ghost/30 flex items-center justify-between px-6 bg-void/90 backdrop-blur-sm">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="relative">
                            ...
                            <input type="text" value={searchFilter} onChange={e => setSearchFilter(e.target.value)} placeholder="Filter tags..."
                                className="h-9 pl-8 pr-8 bg-black/50 border border-ghost/30 rounded text-white text-sm focus:border-signal/50 focus:outline-none font-mono w-44" />
                            {searchFilter && <button onClick={() => setSearchFilter('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-400"><X size={12} /></button>}
                        </div>
                        {/* Import */}
                        <button
                            onClick={() => fileInputRef.current?.click()}
                            className="flex items-center gap-1.5 px-3 py-2 bg-black/50 border border-ghost/30 hover:border-signal/40 text-gray-400 hover:text-signal rounded text-sm transition-colors"
                        >
