...
import { 
    ...
    Verified, Fingerprint, Activity, Phone, Clock
} from 'lucide-react';
...
export function PayloadsList() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToMore({
      ...
      updateQuery: (prev, { subscriptionData }) => {
        ...
        const mergedPayloads = [...existingPayloads];
  ...
  const handleDelete = (uuid: string, deleted: boolean) => {
      toggleDelete({ variables: { payload_uuid: uuid, deleted: !deleted } });
  };
                    ...
                    <AnimatePresence>
                          ...
                          >
                              <div className="py-1">
                                  ...
                                  <button 
                                      onClick={() => { setIsOpen(false); handleDelete(row.uuid, row.deleted); }}
                                      className={cn(
                                          "action-item",
                                          row.deleted ? "text-green-400 hover:bg-green-900/20" : "text-red-400 hover:bg-red-900/20"
                                      )}
                                  >
                                      {row.deleted ? <RotateCcw size={14} /> : <Trash2 size={14} />}
                                      {row.deleted ? "Restore Payload" : "Delete from Disk"}
                                  </button>
                              </div>
                          </motion.div>
                    </AnimatePresence>
            ...
            <button
              onClick={(e) => { 
                  ...
                  if (row.filemetum?.agent_file_id) {
                      window.location.href = `/api/v1.4/files/download/${row.filemetum.agent_file_id}`;
                  }
              }}
