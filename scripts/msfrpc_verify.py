...
# Credentials come from the environment (or .env.msf, which
# scripts/minerva_install.sh generates on first `msf-start`). Nothing is
# hardcoded here: this file is committed, and the password is not.
HOST = os.environ.get("MSFRPC_HOST", "127.0.0.1")
PORT = int(os.environ.get("MSFRPC_PORT", "55553"))
USER = os.environ.get("MSFRPC_USER", "msf")
PASS = os.environ.get("MSFRPC_PASS", "")
if not PASS:
    raise SystemExit(
        "MSFRPC_PASS is not set. Run:  set -a; . /opt/Minerva/.env.msf; set +a\n"
        "(generated by: bash scripts/minerva_install.sh msf-start)"
    )
